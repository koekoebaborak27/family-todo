...
## 2026-09-02 家族にログインしてもらうためのテストユーザー登録手順

Google OAuth が「テスト中」のままなので、ログインする人は全員テストユーザーへの登録が要る（上の節を参照）。手順は次のとおり。コード作業は無く、すべてブラウザ上の操作。

1. Google Cloud コンソールの上部で、**プロジェクト番号 `7906177976`** のプロジェクトを選ぶ。このアプリのクライアントID（`apps/backend/wrangler.jsonc` の `GOOGLE_CLIENT_ID`）の先頭がこの番号。
2. [Google Auth Platform → 対象](https://console.cloud.google.com/auth/audience) を開く。左メニューからは「Google Auth Platform」→「対象」。
3. 画面上部の公開ステータスが「テスト中」であることを確認する。
4. 「テストユーザー」セクションの「ユーザーを追加」から、家族のGoogleアカウントのメールアドレスを1行1件で入力して保存する。

登録するときの注意。

- **実際にログインに使うアドレスそのもの**を入れる。`+` を付けたエイリアスや別名アドレスでは通らない。
- 上限は100人。
- 反映は基本すぐだが、数分かかることがある。
- 要求しているスコープは `openid email profile` だけ（Googleの分類では非機密）なので、「このアプリは Google で確認されていません」の警告画面は出ない想定。

登録した人にやってもらうこと。

1. https://family-todo-frontend.koekoe-app.workers.dev を開き「Googleでログイン」。
2. ログイン後、家族グループに未所属なら作成・参加画面が出るので、招待コードを入力して参加する。

招待コードは、グループ作成者の家族グループ設定画面（`/family/settings`）の「家族を招待する」に表示される。同じ場所から招待リンクをコピーして送るのが確実で、リンクを踏めば未ログインの人でもログイン後に招待コードを引き継いだ状態で参加画面まで進む。

ログイン時にGoogleの「アクセスをブロックしました（このアプリは Google の審査プロセスを完了していません）」が出た場合は、そのアカウントがテストユーザーに入っていないか、アドレスの綴りが違う。
