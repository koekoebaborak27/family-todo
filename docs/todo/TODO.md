...
次の宿題は「単体テストを作成・実行する」。各機能実装時にBackend/Frontendとも`service.ts`等の単体テストは作成済み（`pnpm test`で確認可能）。未着手の観点（境界値・異常系の網羅状況など）の洗い出しから始める。
...
| 項目 | 状態 |
| --- | --- |
| 作業ブランチ | `codex/ui-review`（iOSインストール案内を取り込んだ `main` から作成） |
