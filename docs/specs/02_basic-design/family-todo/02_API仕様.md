...
- 期限接近・期限超過の通知は、ユーザー操作に紐づくAPIではなく、Cloudflare Cron Triggers（15分おき。[インフラ関連事項の技術検証](../README.md#インフラ関連事項の技術検証)で確定）による定期実行から`todos`・`notification_settings`を突き合わせて送信する。対象の絞り込み・重複送信防止・送信失敗時の扱いは[`20_通知バッチ処理.md`](../../03_detail-design/family-todo/20_通知バッチ処理.md)を参照。
