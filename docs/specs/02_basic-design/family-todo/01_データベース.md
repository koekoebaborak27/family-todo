...
Cloudflare D1（SQLite互換）を想定。マイグレーションはPrismaではなくwranglerの機能で管理する（`apps/backend/migrations/README.md`）。物理的なカラム型・インデックスの詳細は、実装時に作成するマイグレーションファイルで確定する。本書はテーブル構成と関連の概要を示す。
...
| due_soon_notified_at | 期限接近通知を送った日時（nullable）。`due_at`が変わると（編集・繰り返しの次回進行・未完了への差し戻し）NULLに戻す。詳細は[`20_通知バッチ処理.md`](../../03_detail-design/family-todo/20_通知バッチ処理.md) |
| overdue_notified_at | 期限超過通知を送った日時（nullable）。リセット条件は上記と同じ |
...
| カラム | 説明 |
| --- | --- |
| id | 主キー。Cookieに保存するセッションIDそのもの（推測不可能なランダム値） |
| user_id | ログイン中のユーザー |
| expires_at | 有効期限。認証付きリクエストのたびに現在時刻の90日後へ延長する（スライディングセッション） |
| created_at | 発行日時 |

- 詳細は[`30_ログインセッション管理.md`](../../03_detail-design/family-todo/30_ログインセッション管理.md)。
